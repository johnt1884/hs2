...
function waitForViewerAndSetupObserver() {
  const observerWait = setInterval(() => {
    const target = document.getElementById("otk-thread-viewer");
    if (target) {
      clearInterval(observerWait);
...
function scrollToAnchoredMessage() {
  if (!otkAnchoredMessageId) return;
  const anchorId = `otk-msg-${otkAnchoredMessageId}`;
  const anchorWait = setInterval(() => {
    const el = document.getElementById(anchorId);
    if (el) {
      clearInterval(anchorWait);
      el.scrollIntoView({ behavior: "auto", block: "center" });
      el.classList.add("otk-anchored-message");
    }
  }, 50);
}
...
function createTweetEmbedElement(tweetId) {
    ...
    if (!otkGuiWrapper) {
        ...
        savingIndicator.id = 'otk-saving-indicator';
        savingIndicator.textContent = 'Saving...';
        savingIndicator.style.cssText = `
            display: none;
            color: var(--otk-background-updates-stats-text-color);
            font-style: italic;
            margin-left: 10px;
        `;
        statsWrapper.appendChild(savingIndicator);
...
function _populateAttachmentDivWithMedia(
    ...
    async function backgroundRefreshThreadsAndMessages(options = {}) { // Added options parameter
        ...
        try {
            ...
            const savingIndicator = document.getElementById('otk-saving-indicator');
...
function processTweetEmbed(embedContainer) {
    ...
    const processTweet = () => {
        // Inner function to handle rendering, preventing code duplication
        const renderTweet = (html) => {
            embedContainer.innerHTML = html;
            if (window.twttr && window.twttr.widgets) {
                // Defer the execution to ensure the DOM is updated from innerHTML.
                // This is the key fix for the race condition.
                setTimeout(() => {
                    try {
                        // Pass the specific container to prevent the widget
                        // from scanning the entire document.
        ...
        // Path for cached tweets
        if (tweetCache[cacheKey]) {
            renderTweet(tweetCache[cacheKey]);
            return;
        }
    ...
    // Logic to load the widgets.js script if not present
